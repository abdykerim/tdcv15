function [f, inlierIdx] = ransac1( x,y,ransacCoef,funcDist )
...
%fLib = cell(1,iterNum);
sample_count = 1;
while sample_count < iterNum
	% 1. fit using  random points
	sampleIdx = randIndex(ptNum,minPtNum);
	f1 = DirectLinearTransformation(x(:,sampleIdx),y(:,sampleIdx));  
	% 2. count the inliers, if more than thInlr, refit; else iterate
	dist = funcDist(f1,x,y);
	inlier1 = find(dist < thDist);
    inlier1
	inlrNum(sample_count) = length(inlier1);
	%if length(inlier1) < thInlr, continue; end
	fLib{sample_count} = DirectLinearTransformation(x(:,inlier1),y(:,inlier1));
    epsil = 1 - (length(inlier1) / ptNum);
    iterNum = log(1 - thInlrRatio)/ log(1 - (1 - epsil)^minPtNum);
