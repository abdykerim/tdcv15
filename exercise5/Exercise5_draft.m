...
tic;
%testing with forest
sizeYOfImage = size(I,1);
sizeXOfImage = size(I,2);
leaf_node_reached = true;
heat_map = zeros(sizeYOfImage, sizeXOfImage);
sizeYOfHeatMap = size(heat_map,1);
sizeXOfHeatMap = size(heat_map,2);
for y = 1:sizeYOfImage
    for x = 1:sizeXOfImage
        sum_px = 0;
        sum_py = 0;
        for tree = 1:1%0
        ...
        avg_px = round(sum_px/1);
        avg_py = round(sum_py/1);
        xIndex = y + avg_px;
        yIndex = x+avg_py;
        if (xIndex> 0 && yIndex > 0 && xIndex <= sizeYOfHeatMap && yIndex <= sizeXOfHeatMap)
...
toc;
% Finding the maximum in the heatmap and representing that point in the
% original image
[rowR, columnR] = find(heat_map == max(max(heat_map)))
    figure, imagesc(I), axis image, colormap(gray), hold on
    plot(columnR,rowR,'gs'), title('SOLUTION');
